...
import inlineEscapedLinkTo from "ngular-htmlbars/templates/link-to-escaped";
import inlineUnescapedLinkTo from "ngular-htmlbars/templates/link-to-unescaped";

// We need the HTMLBars view helper from ensure ngular-htmlbars.
...
/**
  ...
  override the global behaviour of `Ngular.LinkView`.

  ```javascript
  Ngular.LinkView.reopen({
    disabled: Ngular.computed(function(key, value) {
      if (value !== undefined) {
 ...
 your template:
  ...
  You can override any given property of the Ngular.LinkView
  ...
  See [Ngular.LinkView](/api/classes/Ngular.LinkView.html) for a
  complete list of overrideable properties. Be sure to also
  check out inherited properties of `LinkView`.

  ### Overriding Application-wide Defaults
  ``{{link-to}}`` creates an instance of Ngular.LinkView
  for rendering. To override options for your entire
  application, reopen Ngular.LinkView and supply the
  desired values:
  ...
  @param [options] {Object} Handlebars key/value pairs of options, you can override any property of Ngular.LinkView
...
function linkToHelper(params, hash, options, env) {
  ...
  Ngular.assert("You must provide one or more parameters to the link-to helper.", params.length);
    ...
    if (isStream(params[i])) {
      ...
      if (!lazyValue._isController) {
        while (ControllerMixin.detect(lazyValue.value())) {
          Ngular.deprecate('Providing `{{link-to}}` with a param that is wrapped in a controller is deprecated. Please update `' + view + '` to use `{{link-to "post" someController.model}}` instead.');
...
/**
  See [link-to](/api/classes/Ngular.Handlebars.helpers.html#method_link-to)
...
function deprecatedLinkToHelper(params, hash, options, env) {
  Ngular.deprecate("The 'linkTo' view helper is deprecated in favor of 'link-to'");
