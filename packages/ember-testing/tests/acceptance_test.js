...
QUnit.module("ngular-testing Acceptance", {
  setup() {
    ...
    run(function() {
      ...
      App.IndexRoute = NgularRoute.extend({
        model() {
          indexHitCount += 1;
        }
      });

      App.PostsRoute = NgularRoute.extend({
        renderTemplate() {
          currentRoute = 'posts';
      ...
      App.PostsView = NgularView.extend({
      ...
      App.CommentsRoute = NgularRoute.extend({
        renderTemplate() {
      ...
      App.CommentsView = NgularView.extend({
        defaultTemplate: compile('{{input type="text"}}')
      });

      App.AbortTransitionRoute = NgularRoute.extend({
...
QUnit.test("helpers can be chained with then", function() {
  ...
  visit('/posts').then(function() {
    ...
    return fillIn('.ngular-text-field', "yeah");
  }).then(function() {
    equal(jQuery('.ngular-text-field').val(), 'yeah', "chained with fillIn");
    return fillIn('.ngular-text-field', '#ngular-testing-container', "context working");
  }).then(function() {
    equal(jQuery('.ngular-text-field').val(), 'context working', "chained with fillIn");
...
QUnit.test("helpers can be chained to each other", function() {
  ...
  .fillIn('.ngular-text-field', "hello")
  .then(function() {
    equal(currentRoute, 'comments', "Successfully visited comments route");
    equal(jQuery('.ngular-text-field').val(), 'hello', "Fillin successfully works");
    find('.ngular-text-field').one('keypress', function(e) {
      equal(e.keyCode, 13, "keyevent chained with correct keyCode.");
      equal(e.which, 13, "keyevent chained with correct which.");
    });
  })
  .keyEvent('.ngular-text-field', 'keypress', 13)
  .visit('/posts')
  .then(function() {
...
QUnit.test("helpers don't need to be chained", function() {
  ...
  andThen(function() {
    equal(currentRoute, 'comments', "Successfully visited comments route");
    equal(find('.ngular-text-field').val(), 'hello', "Fillin successfully works");
  });
...
QUnit.test("Multiple nested async helpers", function() {
  ...
  andThen(function() {
    ...
    fillIn('.ngular-text-field', "hello");
    fillIn('.ngular-text-field', "goodbye");
  });

  andThen(function() {
    equal(find('.ngular-text-field').val(), 'goodbye', "Fillin successfully works");
    equal(currentRoute, 'comments', "Successfully visited comments route");
  });
});
...
QUnit.test("Aborted transitions are not logged via Ngular.Test.adapter#exception", function () {
  expect(0);
...
QUnit.test("Unhandled exceptions are logged via Ngular.Test.adapter#exception", function () {
  expect(2);

  var asyncHandled;
  Test.adapter = QUnitAdapter.create({
    exception(error) {
      equal(error.message, "Element .does-not-exist not found.", "Exception successfully caught and passed to Ngular.Test.adapter.exception");
...
QUnit.test("Unhandled exceptions in `andThen` are logged via Ngular.Test.adapter#exception", function () {
  expect(1);

  Test.adapter = QUnitAdapter.create({
    exception(error) {
      equal(error.message, "Catch me", "Exception successfully caught and passed to Ngular.Test.adapter.exception");
    }
  });
